...
def Scale_rat_text_compu_scale(ll,ul,num_a,num_b,num_c,den_a,den_b,den_c):#partially completed
	...
	# todo here Textable part is not yet done 
	# first compu scale is for  ax^2+bx+c/Xx^2+Yx+z >> ul and ll
	# and for textable there sholud be enums from second compu scale and which will
	# have its own ul,ll and text value	, symbol is not for 402
...
def Scale_linear_text_compu_scale(ll,ul,num_a,num_b,den_a):#partially completed
	a=processor.value_to_str(ll)
	b=processor.value_to_str(ul)
	c=processor.value_to_str(num_a)
	d=processor.value_to_str(num_b)
	...
	# todo here Textable part is not yet done 
	# first compu scale is for  offset , lsb which are numerator ul and ll and 
	# there will not be any denominator for this because its always 1
	# and for textable there sholud be enums from second compu scale and which will
	# have its own ul,ll and text value	, symbol is not for 402
...
def DataConstr_Int(DataConstr_folder_elements, DataConstr_shortname,ll,ul):#completed
	...
	data_constr_rules=ET.SubElement(data_constr,'DATA-CONSTR-RULES')
	data_constr_rule=ET.SubElement(data_constr_rules,'DATA-CONSTR-RULE')
	int_constrs=ET.SubElement(data_constr_rule,'INTERNAL-CONSTRS')
	lower_limit=ET.SubElement(int_constrs,'LOWER-LIMIT')
	lower_limit.text=a
	upper_limit=ET.SubElement(int_constrs,'UPPER-LIMIT')
	upper_limit.text=b
...
def ImplementationDataType_ArrayVariable(ImplementationDataTypes_folder_elements, IDT_shortname, arraysize_variable, IDT):#partially completed
...
def ImplementationDataType_Structure(ImplementationDataTypes_folder_elements, IDT_shortname):#completed
	global IDT_elements
	a=processor.value_to_str(IDT_shortname)
	implementation_data_type=ET.SubElement(ImplementationDataTypes_folder_elements,'IMPLEMENTATION-DATA-TYPE')
	implementation_data_type.attrib={'UUID': rng.generate_uuid()}
	short_name=ET.SubElement(implementation_data_type,'SHORT-NAME')
	short_name.text=a
...
def ImplementationDataType_Record_elements(IDT_element_shortname, IDT):#completed 
    a=processor.value_to_str(IDT_element_shortname)
...
def internal_behaviors(CurrentInternalBehaviors_shortname,swc_type): #completed
...
def Rnblsymbol(Rnbl_symbol): #completed
...
def dra(): # Ensure DATA-READ-ACCESSS is created only once

    global data_read_accesss

    if 'data_read_accesss' not in globals() or data_read_accesss is None:
...
def dwa ():#completed
    global data_write_accesss
    if 'data_write_accesss' not in globals() or data_write_accesss is None:
        data_write_accesss=ET.SubElement(runnable_entity,'DATA-WRITE-ACCESSS')
...
def drpa(): # 
    ...
    if 'data_receive_point_by_arguments' not in globals() or data_receive_point_by_arguments is None:
...
def drpv (): #completed
    global data_receive_point_by_values
    if 'data_receive_point_by_values' not in globals() or data_receive_point_by_values is None:
        data_receive_point_by_values=ET.SubElement(runnable_entity,'DATA-RECEIVE-POINT-BY-VALUES')
...
def dsp (): #completed 
    global data_send_points
    if 'data_send_points' not in globals() or data_send_points is None:
        data_send_points=ET.SubElement(runnable_entity,'DATA-SEND-POINTS')
...
def IRVRA (): #completed 
    global read_local_variables
    if 'read_local_variables' not in globals() or read_local_variables is None:
        read_local_variables=ET.SubElement(runnable_entity,'READ-LOCAL-VARIABLES')
...
def IRVWA (): #completed
    global written_local_variables
    if 'written_local_variables' not in globals() or written_local_variables is None:
        written_local_variables=ET.SubElement(runnable_entity,'WRITTEN-LOCAL-VARIABLES')
...
def msp(): #completed
    ...
    global mode_switch_points
    if 'mode_switch_points' not in globals() or mode_switch_points is None:
        mode_switch_points=ET.SubElement(runnable_entity,'MODE-SWITCH-POINTS')
...
def pa (): #completed
    global parameter_accesss
    if 'parameter_accesss' not in globals() or parameter_accesss is None:
        parameter_accesss=ET.SubElement(runnable_entity,'PARAMETER-ACCESSS')
...
def sscp () : #completed
    global server_call_points
    if 'server_call_points' not in globals() or server_call_points is None:
        server_call_points=ET.SubElement(runnable_entity,'SERVER-CALL-POINTS')
